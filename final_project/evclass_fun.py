...
from SNSNPy.sql_db import sc, ew, combull, sil
...
stas = ["aal", "arn", "ask", "bac", "bju", "ble", "bog", "bor", "bre", "bur", 
...
stas_hals    = ["fag", "mor", "hog", "sod", "bas", "hal", "sjv",
                "gad", "fus", "ess", "orb", "lus", "dju"]
...
tws = ["Z-P", "Z-Pc", "Z-S", "Z-Sc","R-P", "R-Pc", "R-S", "R-Sc", "T-P", "T-Pc", "T-S", "T-Sc" ]            
cols = []
for i in np.insert(np.arange(2,39,2), 0, 1):
    cols = cols + [str(i)+"-" + x for x in tws]
cols.append("typ")

# Columns for the 2nd layer of the event classifier.
new_cols = ["source_loc", "active_hour", "ex_prob", "qu_prob", "ql_prob", "me_prob", "typ"]
...
def generate_waveformdata(arrT_p, arrT_s, staU, network, ev_lat, ev_lon, plot=False, lf=2.0, hf=20.0):
    f_lo           = np.insert(np.arange(2,39,2), 0, 1)                     # Lower end of pass-band
    ...
    arrT_p_evornot = arrT_p - datetime.timedelta(seconds=diff_time/1.0)     # For the evornot-classification
    arrT_s_evornot = arrT_s - diff_time/1.0                                 # -||-
    ...
    try:
        ...
        if days_diff <= 7:
            (D,n) = swd_client.SwdData(network,staU,getT,dur+1, server=["130.238.140.21", 48673, 1])
        else:
            (D,n) = swd_client.SwdData(network,staU,getT,dur+1, server=["130.238.140.20", 48673, 1])

    # FDSN server
    except:
        try:
            fdsn   = "http://130.238.140.89:8080"
            t1_s   = "%4d-%02d-%02dT%02d:%02d:%06.3f" %(startT.year,startT.month,startT.day,startT.hour,startT.minute,startT.second)
            t1     = UTCDateTime(t1_s)
            t2_s   = "%4d-%02d-%02dT%02d:%02d:%06.3f" %(endT.year,endT.month,endT.day,endT.hour,endT.minute,endT.second)
            t2     = UTCDateTime(t2_s)
            client = Client(fdsn)
            if network in ["UP", "MS"]:

                srate = 100
                stz   = client.get_waveforms(network=network,station=staU,location="*",channel="HH*",starttime=t1,endtime=t2)
                stz.sort()  # ENZ
                D = [
                ...
                try:
                    stz   = client.get_waveforms(network=network,station=staU,location="*",channel="HH*",starttime=t1,endtime=t2,attach_response=True)
                    ch = "HHZ"
                except:
                    stz   = client.get_waveforms(network=network,station=staU,location="*",channel="CH*",starttime=t1,endtime=t2,attach_response=True) 
                    ch = "CHZ"

                srate = int(stz[0].stats.sampling_rate)

                # Identify FIRResponse stages
                inv_res = client.get_stations(network=network,station=staU,channel=ch,starttime=t1,endtime=t2,level="response")
                loc = inv_res[0][0][0].location_code
                if loc:
                    seed_id = network + "." + inv_res[0][0].code + "." + loc + "." + inv_res[0][0][0].code   
                else:
                    seed_id = network + "." + inv_res[0][0].code + ".." + inv_res[0][0][0].code
                ...
                if FIR_stage_found:
                    stz.remove_response(output="vel", pre_filt=pfilt, zero_mean=True, taper=True, taper_fraction=0.05, end_stage=end_stage) 
                else:
                    stz.remove_response(output="vel", pre_filt=pfilt, zero_mean=True, taper=True, taper_fraction=0.05)   

                if srate != int(stz[0].stats.sampling_rate):
                    print(staU, " -- Sampling rate changed after removal of instrument response")

                inv       = client.get_stations(network=network,station=staU,starttime=t1,endtime=t2,level="channel")
                sta_lat   = inv[0][0].latitude ; sta_lon = inv[0][0].longitude
                sta_coord = [sta_lat*d2r, sta_lon*d2r]
                D = LocationDist(2,ev_coord,sta_coord, -1)
                baz = D[2]/d2r
                stz.rotate(method="NE->RT", back_azimuth=baz)
                stz[0].data *= 1e9 ; stz[1].data *= 1e9 ; stz[2].data *= 1e9
    ...
    K = int(k/100 * srate)
    del D[0][0:K] ; del D[1][0:K] ; del D[2][0:K]
    del D[0][-1-srate+K:-1] ; del D[1][-1-srate+K:-1] ; del D[2][-1-srate+K:-1] 

    if FDSN:
        del D[0][-2:] ; del D[1][-2:] ; del D[2][-2:]
    else:
        del D[0][-1:] ; del D[1][-1:] ; del D[2][-1:]
    ...
    for j in range(len(f_lo)):
        ...
        st_copy.filter("bandpass", freqmin=f_lo[j], freqmax=f_hi)    # Filter in different frequency bands 
        ...
        # Event-or-not
        evornot_data[j*n_feats+0] = rms(Est_P["Z"]) ; evornot_data[j*n_feats+1] = rms(Est_Pc["Z"]) ; evornot_data[j*n_feats+2]  = rms(Est_S["Z"]) ; evornot_data[j*n_feats+3]  = rms(Est_Sc["Z"])
        evornot_data[j*n_feats+4] = rms(Est_P["R"]) ; evornot_data[j*n_feats+5] = rms(Est_Pc["R"]) ; evornot_data[j*n_feats+6]  = rms(Est_S["R"]) ; evornot_data[j*n_feats+7]  = rms(Est_Sc["R"])
        evornot_data[j*n_feats+8] = rms(Est_P["T"]) ; evornot_data[j*n_feats+9] = rms(Est_Pc["T"]) ; evornot_data[j*n_feats+10] = rms(Est_S["T"]) ; evornot_data[j*n_feats+11] = rms(Est_Sc["T"])

        # Evclass
        evclass_data[j*n_feats+0] = rms(st_P["Z"]) ; evclass_data[j*n_feats+1] = rms(st_Pc["Z"]) ; evclass_data[j*n_feats+2]  = rms(st_S["Z"]) ; evclass_data[j*n_feats+3]  = rms(st_Sc["Z"])
        evclass_data[j*n_feats+4] = rms(st_P["R"]) ; evclass_data[j*n_feats+5] = rms(st_Pc["R"]) ; evclass_data[j*n_feats+6]  = rms(st_S["R"]) ; evclass_data[j*n_feats+7]  = rms(st_Sc["R"])
        evclass_data[j*n_feats+8] = rms(st_P["T"]) ; evclass_data[j*n_feats+9] = rms(st_Pc["T"]) ; evclass_data[j*n_feats+10] = rms(st_S["T"]) ; evclass_data[j*n_feats+11] = rms(st_Sc["T"])
...
def remove_IR(st):
    for s in st:                                            # Use the metadata server
        sta = s.stats.station
        net = s.stats.network
        meta,err = smd_client.load_smd(station=sta,t=s.stats.starttime,network=net,cmplx_ret=True)
        if err:
        ...
        s.simulate(paz_remove=paz,paz_simulate=None,pre_filt=pfilt,zero_mean=True,taper=True,taper_fraction=0.05,pitsasim=False,sacsim=True)
...
def rotate_zrt(st, ev_coord):
    ...
    for s in st:
        ...
        sta_coord = [meta[net][sta][0][1]*d2r,meta[net][sta][0][2]*d2r,0.]
        D = LocationDist(2,ev_coord,sta_coord, -1)
...
def extract_windows(st, slices):
    n_channels = len(st)
    st_N={} ; st_P={} ; st_Pc={} ; st_S={} ; st_Sc={} ;st_C={}              # Dictionaries to store the data
...
def plot_traces(st, slices, sta, startT, lf=2.0, hf=20.0):
    ...
    st_copy = st.copy() 
    st_copy.filter("bandpass", freqmin=lf, freqmax=hf)                                          # Filter in different frequency bands
    stc_N,stc_P,stc_Pc,stc_S,stc_Sc,stc_C = extract_windows(st_copy, slices)                    # Extract the filtered windows

    try:
        max_val = max([ max(stc_N["Z"][100:]), max(stc_P["Z"][100:]), max(stc_Pc["Z"][100:]), max(stc_S["Z"][100:]), max(stc_Sc["Z"][100:]), max(stc_C["Z"][100:]),                         
                        max(stc_N["R"][100:]), max(stc_P["R"][100:]), max(stc_Pc["R"][100:]), max(stc_S["R"][100:]), max(stc_Sc["R"][100:]), max(stc_C["R"][100:]),
                        max(stc_N["T"][100:]), max(stc_P["T"][100:]), max(stc_Pc["T"][100:]), max(stc_S["T"][100:]), max(stc_Sc["T"][100:]), max(stc_C["T"][100:]) ])

        min_val = min([ min(stc_N["Z"][100:]), min(stc_P["Z"][100:]), min(stc_Pc["Z"][100:]), min(stc_S["Z"][100:]), min(stc_Sc["Z"][100:]), min(stc_C["Z"][100:]),                        
                        min(stc_N["R"][100:]), min(stc_P["R"][100:]), min(stc_Pc["R"][100:]), min(stc_S["R"][100:]), min(stc_Sc["R"][100:]), min(stc_C["R"][100:]),
                        min(stc_N["T"][100:]), min(stc_P["T"][100:]), min(stc_Pc["T"][100:]), min(stc_S["T"][100:]), min(stc_Sc["T"][100:]), min(stc_C["T"][100:]) ])
        minmax = False
    except:
        minmax = False
    ...
    pick_height = 1.0
    ax1.set_title("Station %s. Filtered %.1f - %.1f Hz" %(sta.upper(), lf, hf), fontsize=25)
    ax1.plot(times[:-1],np.concatenate((stc_N["Z"],stc_P["Z"],stc_Pc["Z"],stc_S["Z"],stc_Sc["Z"],stc_C["Z"])),"k")
    if minmax:
        ax1.set_ylim([min_val, max_val])
    ymin,ymax = ax1.get_ylim()
    ax1.vlines(times[slices["p_ind"]], ymin*pick_height, ymax*pick_height,    color=colp, linewidth=2, linestyle="dashed")
    ax1.annotate("P",((times[slices["p_ind"]] + times[slices["pc_ind"]])/2,ymax*0.8),color="r",fontsize=fs)
    ax1.vlines(times[slices["pc_ind"]], ymin*pick_height, ymax*pick_height,   color=colp, linewidth=2, linestyle="dashed")
    ax1.annotate("Pc",((times[slices["pc_ind"]] + times[slices["s_ind"]])/2,ymax*0.8),color="r",fontsize=fs)
    ax1.vlines(times[slices["s_ind"]], ymin*pick_height, ymax*pick_height,    color=cols, linewidth=2, linestyle="dashed")
    ax1.annotate("S",((times[slices["s_ind"]] + times[slices["sc_ind"]])/2,ymax*0.8),color="b",fontsize=fs)
    ax1.vlines(times[slices["sc_ind"]], ymin*pick_height, ymax*pick_height,   color=cols, linewidth=2, linestyle="dashed")
    ax1.annotate("Sc",((times[slices["sc_ind"]] + times[slices["coda_ind"]])/2,ymax*0.8),color="b",fontsize=fs)
    ax1.vlines(times[slices["coda_ind"]], ymin*pick_height, ymax*pick_height, color=colc, linewidth=2, linestyle="dashed")
    ax1.set_xlim([0,180])

    ax2.plot(times[:-1],np.concatenate((stc_N["R"],stc_P["R"],stc_Pc["R"],stc_S["R"],stc_Sc["R"],stc_C["R"])),"k")
    if minmax:
        ax2.set_ylim([min_val, max_val])
    ymin,ymax = ax2.get_ylim()
    ax2.vlines(times[slices["p_ind"]], ymin*pick_height, ymax*pick_height, color=colp, linewidth=2, linestyle="dashed")
    ax2.annotate("P",((times[slices["p_ind"]] + times[slices["pc_ind"]])/2,ymax*0.8),color="r",fontsize=fs)
    ax2.vlines(times[slices["pc_ind"]], ymin*pick_height, ymax*pick_height, color=colp, linewidth=2, linestyle="dashed")
    ax2.annotate("Pc",((times[slices["pc_ind"]] + times[slices["s_ind"]])/2,ymax*0.8),color="r",fontsize=fs)
    ax2.vlines(times[slices["s_ind"]], ymin*pick_height, ymax*pick_height, color=cols, linewidth=2, linestyle="dashed")
    ax2.annotate("S",((times[slices["s_ind"]] + times[slices["sc_ind"]])/2,ymax*0.8),color="b",fontsize=fs)
    ax2.vlines(times[slices["sc_ind"]], ymin*pick_height, ymax*pick_height, color=cols, linewidth=2, linestyle="dashed")
    ax2.annotate("Sc",((times[slices["sc_ind"]] + times[slices["coda_ind"]])/2,ymax*0.8),color="b",fontsize=fs)
    ax2.vlines(times[slices["coda_ind"]], ymin*pick_height, ymax*pick_height, color=colc, linewidth=2, linestyle="dashed")

    ax3.plot(times[:-1],np.concatenate((stc_N["T"],stc_P["T"],stc_Pc["T"],stc_S["T"],stc_Sc["T"],stc_C["T"])),"k")
    if minmax:
        ax3.set_ylim([min_val, max_val])
    ymin,ymax = ax3.get_ylim()
    ax3.vlines(times[slices["p_ind"]], ymin*pick_height, ymax*pick_height, color=colp, linewidth=2, linestyle="dashed")
    ax3.annotate("P",((times[slices["p_ind"]] + times[slices["pc_ind"]])/2,ymax*0.8),color="r",fontsize=fs)
    ax3.vlines(times[slices["pc_ind"]], ymin*pick_height, ymax*pick_height, color=colp, linewidth=2, linestyle="dashed")
    ax3.annotate("Pc",((times[slices["pc_ind"]] + times[slices["s_ind"]])/2,ymax*0.8),color="r",fontsize=fs)
    ax3.vlines(times[slices["s_ind"]], ymin*pick_height, ymax*pick_height, color=cols, linewidth=2, linestyle="dashed")
    ax3.annotate("S",((times[slices["s_ind"]] + times[slices["sc_ind"]])/2,ymax*0.8),color="b",fontsize=fs)
    ax3.vlines(times[slices["sc_ind"]], ymin*pick_height, ymax*pick_height, color=cols, linewidth=2, linestyle="dashed")
    ax3.annotate("Sc",((times[slices["sc_ind"]] + times[slices["coda_ind"]])/2,ymax*0.8),color="b",fontsize=fs)
    ax3.vlines(times[slices["coda_ind"]], ymin*pick_height, ymax*pick_height, color=colc, linewidth=2, linestyle="dashed")

    ax3.set_xlabel("Time after %s [s]" %startT, fontsize=fs)
    ax1.set_ylabel("Velocity [nm/s] \n\n Z", fontsize=fs) ; ax2.set_ylabel("Velocity [nm/s] \n\n R", fontsize=fs)
...
def compute_Sarrival(ev_OT, dist, ev_dep):
    ...
    if False:
        ...
        s_vels = [3.58, 3.58, 3.81, 3.81, 4.10, 4.10, 4.65, 4.65, 4.77, 4.77, 4.91,   7.37] 
...
def kiru_active_hour(OT):
    ...
    hr = int(OT.hour)
    mi = int(OT.minute)
    return (dst and hr == 23 and mi >= 10 and mi <= 40) or (not dst and hr == 0 and mi >= 10 and mi <= 40)
...
def malm_active_hour(OT):
    ...
    hr = int(OT.hour)
    mi = int(OT.minute)
    return (dst and ((hr == 22 and mi <= 30) or (hr == 21 and mi > 55))) or (not dst and ((hr == 23 and mi <= 30) or (hr == 22 and mi > 55)))


def dist_kiru(ev_lat, ev_lon):
    dist = LocationDist(2, [ev_lat, ev_lon],[mines["kiru"][0]*d2r, mines["kiru"][1]*d2r], -1)[0]
    return dist


def dist_malm(ev_lat, ev_lon):
    dist = LocationDist(2, [ev_lat, ev_lon],[mines["malm"][0]*d2r, mines["malm"][1]*d2r], -1)[0]
    return dist


def dist_aitik(ev_lat, ev_lon):
    dist = LocationDist(2, [ev_lat, ev_lon],[mines["aiti"][0]*d2r, mines["aiti"][1]*d2r], -1)[0]
    return dist
...
def in_kiru(ev_lat, ev_lon, sil=False):
    ...
    dist_mert = LocationDist(2, [ev_lat, ev_lon],[mines["mert"][0]*d2r, mines["mert"][1]*d2r], -1)[0]
    dist_svap = LocationDist(2, [ev_lat, ev_lon],[mines["svap"][0]*d2r, mines["svap"][1]*d2r], -1)[0]
...
def in_malm(ev_lat, ev_lon, sil=False):
    ...
    dist_aiti = LocationDist(2, [ev_lat, ev_lon],[mines["aiti"][0]*d2r, mines["aiti"][1]*d2r], -1)[0]
...
def in_ren_kan_bjo(ev_lat, ev_lon):
    dist_lim = 3
    dist_rens = LocationDist(2, [ev_lat, ev_lon],[mines["rens"][0]*d2r, mines["rens"][1]*d2r], -1)[0]
    dist_kank = LocationDist(2, [ev_lat, ev_lon],[mines["kank"][0]*d2r, mines["kank"][1]*d2r], -1)[0]
    dist_bjor = LocationDist(2, [ev_lat, ev_lon],[mines["bjor"][0]*d2r, mines["bjor"][1]*d2r], -1)[0]
        ...
        if dist < 100 and sta not in [x[:-1] for x in phases]:
            ...
            for sta_phase in [x[:-1] for x in phases]:
                if sta_phase in stas_old:
                    continue
                sta_phase_coord = [float(sta_d[sta_phase][0])*d2r, float(sta_d[sta_phase][1])*d2r]
...
def compute_probabilities(score_ex, score_qu, score_ql, score_me, weights):
    ...
    probs = np.zeros(4)
    k = 0
    for l in [score_ex, score_qu, score_ql, score_me]:
        probs[k] = np.average(l, weights=weights)
        k += 1
    #print(np.sum(probs))    
    return probs
...
def str2datetime(date):
    ...
    try:
        ...
        print("Something wrong in date string. Format should be <yyyymmdd>")
...
def get_evlib_path(BASE_PATH, date):
    return BASE_PATH + "/" + str(date.year) + "/" + str.lower(date.strftime("%b")) + "/" + str(date.strftime("%d")) + "/" + "events.lib"


def get_evaut_path(BASE_PATH, date):
    return BASE_PATH + "/" + str(date.year) + "/" + str.lower(date.strftime("%b")) + "/" + str(date.strftime("%d")) + "/" + "events.aut"
...
def sta_dict():
    ...
    for line in lines:
        lin = line.split()
        try:
            sta = lin[0] ; lat = lin[1] ; lon = lin[2] ; dep = lin[3] ; site = lin[4]
        except:
            continue
...
def evclass_write_grx_sta(sta, start_date, end_date, max_dist, Kiru, Malm, grx_file):
    ...
    while curr_date >= start_date:
        ...
        for ev in ev_lib:
            ...
            typ       = ev["type"]
            if typ not in typs_poss or (count_typs[np.where(typs_poss==typ)[0][0]] >= lim):
                continue
                    ...
                    # Case 2 - Training a model with events associated with the mine in Kiruna.
                        ...
                        If:
                            ...
                            * The count for the event type in question has exceeded the given limit
                            ==> continue
                        """
                        if (not kiru) or (typ not in typs_poss) or (typ != "ex" and hr in [0,23]) or (count_typs[np.where(typs_poss==typ)[0][0]] >= lim):
                            continue
                    ...
                    # Case 3 - Training a model with events associated with the mine in Malmberget.
                        ...
                        If:
                            ...
                            * The count for the event type in question has exceeded the given limit
                            ==> continue
                        """
                        if (not malm) or (typ not in typs_poss) or (typ != "ex" and hr in [22,23]) or (count_typs[np.where(typs_poss==typ)[0][0]] >= lim):
                            continue
...
def evclass_data_pprocess(sta, grx_file, wf_file):
    ...
    for i in range(0, n_evs):
        ...
        try:
            tmp[0,:-1] = generate_waveformdata(arrT_p=arrT_p,arrT_s=arrT_s,staU=staU,network="UP",ev_lat=ev_coord[0],ev_lon=ev_coord[1])[1]
            tmp[0,-1]  = int(typ)
            data       = np.append(data, tmp, axis=0)
        except:
            print("Problem generating waveform data") 
            continue
...
def evclass_train_model(sta, data, Kiru, Malm, save_dir):
    ...
    # Case 2 - Training a model with events associated with the mines in Kiruna/Malmberget.
        ...
        # Compile the model
        model.compile(                                                                                             
            optimizer=keras.optimizers.Adam(1e-4), loss="categorical_crossentropy", metrics=metrics
        )
...
def classification_map(ev_lat, ev_lon, seen, dists, sta_pred, pred_typ, conf, distance):
    ...
    rang_lat = 2.5
    rang_lon = 5.5
    fig.coast(shorelines=False, 
            region=[ev_lon-rang_lon,ev_lon+rang_lon,ev_lat-rang_lat,ev_lat+rang_lat], 
            projection="M20c", 
            water="lightskyblue",
            land="oldlace",
            borders="1/1p",
    ...
    sta_typ = [] ; sta_conf  = [] ; sta_lat = [] ; sta_lon = [] ; sta_cod = [] ; sta_alt = []
    # Stations with no phase picked
    sta_lat_n = [] ; sta_lon_n = [] ; sta_cod_n = [] ; sta_alt_n = []
    # Stations with a phase picked but no prediction
    sta_typ_e = [] ; sta_lat_e = [] ; sta_lon_e = [] ; sta_cod_e = [] ; sta_alt_e = []
    # Stations with a phase picked but too far awya
    sta_cod_d = [] ; sta_lat_d = [] ; sta_lon_d = []
    for sta in stalist:
        st = sta.split()
        cod = st[0]
    ...
    s = {"code" : sta_cod, "latitude" : sta_lat, "longitude" : sta_lon, "altitude" : sta_alt, "type" : sta_typ, "confidence": sta_conf}
    station = pd.DataFrame(data=s)

    s_n = {"code" : sta_cod_n, "latitude" : sta_lat_n, "longitude" : sta_lon_n, "altitude" : sta_alt_n}
    station_n = pd.DataFrame(data=s_n)

    s_e = {"code" : sta_cod_e, "latitude" : sta_lat_e, "longitude" : sta_lon_e, "altitude" : sta_alt_e, "type" : sta_typ_e}
    ...
    """
    earth_radius     = 6371.0
    angular_distance = (dist_max / earth_radius) * (180 / np.pi)
    delta_lon        = angular_distance / np.cos(np.deg2rad(ev_lat))
    end_lon          = ev_lon + delta_lon
    if distance == []:
        rad = 150
    else:
        rad = np.max(distance)
...
def in_combull(ev_id):
    ...
    for ev in evs:
        ...
        # Epicentral distance
        dist_cbl = LocationDist(2, [evaut_lat*d2r, evaut_lon*d2r], [evcbl_lat*d2r, evcbl_lon*d2r], -1)[0]
...
def in_silbull(ev_id):
    ...
    start = datetime.datetime(evaut_OT.year, evaut_OT.month, evaut_OT.day, 0, 0, 0)-datetime.timedelta(days=1)
    end   = datetime.datetime(evaut_OT.year, evaut_OT.month, evaut_OT.day, 23, 59, 59)+datetime.timedelta(days=1)
    ...
    for ev in evs:
        ...
        # Epicentral distance
        dist_sil = LocationDist(2, [evaut_lat*d2r, evaut_lon*d2r], [evsil_lat*d2r, evsil_lon*d2r], -1)[0]
